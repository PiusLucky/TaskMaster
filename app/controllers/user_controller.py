...
def createUserController():
    try:
        ...
        passwordForm = PasswordForm(data={"password": json_data.get("password")})
...
def loginUserController():
    try:
        ...
        passwordInstance = PasswordModel.query.filter_by(user_id=userId).first()

        # Convert the hexadecimal string to bytes
        bytes_hashed_password = bytes.fromhex(passwordInstance.password[2:])  # Remove the leading "\\x"
        ...
        return error_response("Invalid credentialss", 401)
